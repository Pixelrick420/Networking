...
int main() {
    ...
    printf("[GBN] Sender waiting on port %d... (window=%d, frames=%d)\n\n",
           PORT, WINDOW, FRAMES);
    ...
    while (base < FRAMES) {
        ...
        if (strncmp(buf, "ACK", 3) == 0) {
            int ack = atoi(buf + 4);
            printf("[GBN] Sender: Received cumulative ACK %d\n\n", ack);
            base = ack + 1;
        } else if (strncmp(buf, "NAK", 3) == 0) {
            int nak = atoi(buf + 4);
            printf("[GBN] Sender: Received NAK %d → Go-Back, retransmitting "
                   "from frame %d\n\n",
                   nak, nak);
            base = nak;
        }
    ...
    printf("[GBN] Sender: All %d frames acknowledged. Done.\n", FRAMES);
    freeServer(server);
    return 0;
}
